...
class LessonAttachmentsManager extends StatefulWidget {
  ...
  @override
  State<LessonAttachmentsManager> createState() => LessonAttachmentsManagerState();
}
...
class LessonAttachmentsManagerState extends State<LessonAttachmentsManager> {
  ...
  Future<void> uploadPendingAttachments(BuildContext context, String lessonId) async {
    ...
    for (final attachment in List<LessonAttachmentEntity>.from(_attachments)) {
      ...
      subscription = cubit.stream.listen((state) {
        if (state is AttachmentUploadSuccess || state is AttachmentUploadError) {
          subscription.cancel();
  ...
  void _handleAttachmentUploadState(BuildContext context, AttachmentUploadState state) {
  ...
  void _handleLessonAttachmentState(BuildContext context, LessonAttachmentState state) {
  if (state is LessonAttachmentUpdated) {
    setState(() {
      final index = _attachments.indexWhere((e) => e.id == state.attachment.id);
      if (index != -1) {
    ...
    final message = state.errors.isNotEmpty ? state.errors.first : 'حدث خطأ غير متوقع';
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
